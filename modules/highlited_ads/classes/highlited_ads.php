...
class highlited_ads {
	...
	function highlited_ads() {

		global $config_table_prefix;
		$this->settings_table = $config_table_prefix."highlited_ads";
	...
	function getSettings($overwrite=0) {
		...
		$lc_cache = new cache();
		if($overwrite || !$pa_settings = $lc_cache->readCache("mod_ha_settings")) {

			global $db;
			$ha_settings = $db->fetchAssoc("select * from ".$this->settings_table);
			$lc_cache->writeCache("mod_ha_settings", $ha_settings);
	...
	function saveSettings() {
		...
		// clear cache
		$lc_cache = new cache();
		$lc_cache->clearCache("mod_ha_settings");

		return 0;
	}


	function getHighlited($hsettings) {
		...
		$start=0;
		$ads_per_page = $hsettings['no_ads'];
		$where = " where ".TABLE_ADS.".active = 1 and ".TABLE_ADS.".highlited =1 ";
                $where .= $locations_str;
