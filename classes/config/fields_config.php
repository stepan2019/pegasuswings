...
class fields_config {
	...
	function delete($id='') {
		...
		} elseif ($type=="image" || $type=="file" || $type=="video" || $type=="audio") {
	...
	function check_form ($id='', $type='') {
		...
		$array_validation = array ( "url" , "email" , "file" , "image" , "date", "youtube", "twitter", "video", "audio" );
		...
		if(($type=='image' || $type=='file' || $type=='video' || $type=='audio') && !$_POST['max_uploaded_size']) {
		...
		if(isset($_POST['image_resize']) && $_POST['image_resize'] && $type=='image' && !validator::valid_length($_POST['image_resize']))
	...
	function add() {
		...
		if($this->clean['type']=="image" && isset($_POST['image_resize'])) $this->clean['image_resize']=escape($_POST['image_resize']); else $this->clean['image_resize']='';
		...
		if($this->clean['type']=="image" || $this->clean['type']=="file" || $this->clean['type']=="video" || $this->clean['type']=="audio") {
	...
	function edit($id) {
		...
		if( $type=="image" && isset($_POST['image_resize'])) $this->clean['image_resize']=escape($_POST['image_resize']); else $this->clean['image_resize']='';
...
//echo $sql;
	...
	function generateDbFieldType($type, $is_numeric) {
		...
		if($type=='radio' || $type=='radio_group' || $type=='menu' || $type=='file' || $type=='image') return "varchar(128) default null";
...
//!!!!!!!!!
			...
			// max uploaded size
			$array_files = array("file", "image", "video", "audio");
			...
			// resize image
			if($array_fields[$i]['type']=="image" && $array_fields[$i]['image_resize']) {
